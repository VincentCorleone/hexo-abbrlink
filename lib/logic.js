...
let logic = function(data) {
    var log = this.log;
    var baseStr = data.date.year() + "/" + data.date.month() + "/" + data.date.day() + "/" + data.slug
    if (data.layout == 'post') {
        let abbrlink
        ...
        if (!abbrlink) {
			...
			let res = (opt_alg == 'crc32' ? crc32.str(baseStr) >>> 0 : crc16(baseStr) >>> 0);
            ...
            log.i("Generate link %s for post [%s]", abbrlink, baseStr);
        }
        model.add(abbrlink);
    }
    return data
